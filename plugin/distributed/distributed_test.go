...
func TestDistributedPlugin(t *testing.T) {
	...
	// Call getQueries
	resp, err := plugin.Call(context.Background(), osquery.ExtensionPluginRequest{"action": "getQueries"})
	...
	// Call writeResults
	getCalled = false
	resp, err = plugin.Call(context.Background(), osquery.ExtensionPluginRequest{"action": "writeResults", "results": `{"queries":{"query1":[{"iso_8601":"2017-07-10T22:08:40Z"}],"query2":[{"version":"2.4.0"}]},"statuses":{"query1":"0","query2":"0","query3":"1"}}`})
...
func TestDistributedPluginErrors(t *testing.T) {
	...
	_, err = plugin.Call(context.Background(), osquery.ExtensionPluginRequest{"action": "getQueries"})
	assert.True(t, getCalled)
	assert.False(t, writeCalled)
	assert.Error(t, err)
	assert.Equal(t, "error getting queries: getQueries failed", err.Error())

	getCalled = false

	// Call with good action but writeResults fails
	// Error unmarshalling results
	_, err = plugin.Call(context.Background(), osquery.ExtensionPluginRequest{"action": "writeResults", "results": "foobar"})
	assert.False(t, getCalled)
	assert.False(t, writeCalled)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "error unmarshalling results")

	// Error converting status
	_, err = plugin.Call(context.Background(), osquery.ExtensionPluginRequest{"action": "writeResults", "results": `{"statuses": {"query1": "foo"}}`})
	assert.False(t, getCalled)
	assert.False(t, writeCalled)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), `error unmarshalling results: json: cannot unmarshal`)

	// Error unmarshalling results
	_, err = plugin.Call(context.Background(), osquery.ExtensionPluginRequest{"action": "writeResults", "results": "{}"})
	assert.False(t, getCalled)
	assert.True(t, writeCalled)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "error writing results")
}
...
func TestHandleResults(t *testing.T) {
	...
	_, err := plugin.Call(context.Background(), osquery.ExtensionPluginRequest{"action": "writeResults", "results": rawJsonQuery})
